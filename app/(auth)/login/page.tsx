...
function LoginContent() {
  ...
  // 1. Grab the 'next' parameter from the URL (e.g., /trip/123)
  const nextPath = searchParams.get("next");

  async function handleGoogleLogin() {
    // 2. Build the base callback URL
    const redirectUrl = new URL(`${window.location.origin}/auth/callback`);
    
    // 3. If a 'next' path exists, attach it to the callback URL
    if (nextPath) {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: redirectUrl.toString(), // e.g., http://localhost:3000/auth/callback?next=/trip/123
      },
    });
...
// Wrap the client component in Suspense so Next.js doesn't fail the build
