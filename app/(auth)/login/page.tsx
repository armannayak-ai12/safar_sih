...
function LoginForm() {
  ...
  const next = searchParams.get("next") || "/suggestor";
  const [isLoading, setIsLoading] = useState(false);

  const handleGoogleLogin = () => {
    setIsLoading(true);
    // Set session cookie (valid for 7 days)
    document.cookie = "safar_session=true; path=/; max-age=604800; SameSite=Lax";
    // Navigate to the requested protected feature URL
